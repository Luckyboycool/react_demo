...
const DictTable = ({ user }: { user: any }) => {
  const dispatch = useDispatch();
  const system_user = useSelector((state: any) => state.system_user);
  const { dataModalVisible } = system_user;

  const [data, { total, loadData, delData }] = useData();
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);
  const initQuery = { current: 1, pageSize: 20, field: '', order: '', dictType: user.dictType || '' };
  const [query, setQuery] = useState(initQuery);
  const [selectData, setSelectData] = useState(null);

  const onCancel = () => {
    dispatch({ type: 'system_user/dictTableVisible', payload: false });
  };
  ...
  const onCreateDict = () => {
    setSelectData(null);
    dispatch({ type: 'system_user/dataModalVisible', payload: true });
  };
  ...
  return (
    <Modal getContainer={false} width={1200} title="Dict Data" footer={null} visible={true} onCancel={onCancel}>
      <StyleContent>
        ...
        />
        {dataModalVisible ? <DataModal selectData={selectData} dictType={user.dictType} reload={onSubmit} /> : null}
      </StyleContent>
    </Modal>
  );
};
